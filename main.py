...
if is_raspberry_pi():
    ...
    filename, _ = QFileDialog.getOpenFileName(
        caption='Open file', dir='.', filter='*.mp4')
    dataFileName = filename.split(".")[0]
    vs = VideoStream(filename, resolution=(WIDTH, HEIGHT), monochrome=True)
dataLogger.setFilename(
    dataFileName + "_" + str(int(time.time())) + ".csv")
# Start video stream
...
processor.signals.result.connect(statsComputer.update)
processor.signals.resultBlobs.connect(tracker.update)
processor.signals.result.connect(dataLogger.update)
...
window.adaptiveThresholdOffsetSpinbox.valueChanged.connect(
    processor.detector.setOffset)
window.adaptiveThresholdBlocksizeSpinBox.valueChanged.connect(
    processor.detector.setBlockSize)
...
statsComputer.signals.finished.connect(
    lambda: window.updatePlot(1, None, statsComputer.area_histogram))
statsComputer.signals.finished.connect(lambda: window.updatePlot(
    2, None, statsComputer.peri_to_area_histogram))
...
dataLogger.signals.message.connect(window.print_output)
...
dataLogger.signals.error.connect(window.error_output)
...
processor.detector.setBlockSize(
    window.adaptiveThresholdBlocksizeSpinBox.value())
...
# window.signals.finished.connect(tracker.stop)
window.signals.finished.connect(dataLogger.save(
    dataFileName + "_" + str(int(time.time())) + ".csv"))
if is_raspberry_pi():
